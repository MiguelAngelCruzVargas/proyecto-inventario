...
class SistemaLecheria(Frame):
    ...
    def create_widgets(self):
        ...
        Label(self.frame2, text='Sobres Disponibles:', fg='white', bg='navy', font=('Rockwell', 13, 'bold')).grid(column=0, row=6, pady=15)
        Label(self.frame2, text=str(self.sobres_disponibles), fg='white', bg='navy', font=('Rockwell', 13, 'bold')).grid(column=1, row=6, pady=15)
    ...
    def create_tabla(self):
        ...
        self.tabla['columns'] = ('Nombre', 'Estado', 'Cantidad', 'Total Inscritos', 'Total No Inscritos')
    ...
    def actualizar_sobres_disponibles_label(self):
        self.frame2.grid_slaves(row=6, column=1)[0].configure(text=str(self.sobres_disponibles))
    ...
    def mostrar_inscritos(self):
        ...
        for nombre, cantidad in self.inscritos.items():
            total_inscritos = sum(self.inscritos.values())
            self.tabla.insert('', 'end', values=(nombre, 'Inscrito', cantidad, total_inscritos, 0))

    def mostrar_no_inscritos(self):
        self.tabla.delete(*self.tabla.get_children())
        for nombre, cantidad in self.no_inscritos.items():
            total_no_inscritos = sum(self.no_inscritos.values())
            self.tabla.insert('', 'end', values=(nombre, 'No Inscrito', cantidad, 0, total_no_inscritos))
